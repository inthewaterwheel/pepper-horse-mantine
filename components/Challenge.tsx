...
function Challenge({ isOpen, onClosedCallback, params }: ChallengeProps) {
  ...
  if (needsNewDraw) {
    const selKeys = Object.keys(SoundData)
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setTimeout(() => {
        playSoundEffect(SoundData[targetKey], 1);
      }, 1200);
    ...
    if (key == targetKey) {
      ...
      if (log) {
        ...
        fetch('/api/log', {
          ...
          body: JSON.stringify({
            target: log.target,
            metadata: { ...log.metadata, timeElapsed: Date.now() - startTime },
          }),
        });
      }
    } else {
      ...
      if (log) {
        setLog({
          target: log.target,
          metadata: { ...log.metadata, incorrectGuesses: (log.metadata.incorrectGuesses ?? 0) + 1 },
        });
      }
    }
  }
